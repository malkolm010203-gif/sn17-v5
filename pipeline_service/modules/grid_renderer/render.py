...
from schemas.image_convertions import pil_to_image_tensor
from schemas.types import ImageTensor
from .schemas import GridRendererInput, GridRendererOutput


class GridViewRenderer():

    def render_grids(self, request: GridRendererInput) -> Optional[GridRendererOutput]:
        grids = []
        for glb_bytes in request.glb_bytes:
            grid = self._grid_from_glb_bytes(glb_bytes)
            if grid is None:
                return None
            grids.append(grid)
        return GridRendererOutput(grids=grids)

    def _grid_from_glb_bytes(self, glb_bytes: bytes) -> Optional[ImageTensor]:
        try:
            ...
            grid_tensor = pil_to_image_tensor(grid)
            logger.info(f"GLB rendering complete, {view_count} views | Generation time: {time.time() - start_time:.2f}s")
            return grid_tensor
